...
CRAWL_GIT_REPO="${CRAWL_GIT_REPO:-https://github.com/electroneum/discv4-dns-lists.git}"
...
update_devp2p_tool() {
	git_update_repo https://github.com/electroneum/electroneum-sc "$etnsc_src"
	(cd "$etnsc_src" && go build ./cmd/devp2p)
}
...
while true; do
	# Pull changes from electroneum-sc
	update_devp2p_tool
